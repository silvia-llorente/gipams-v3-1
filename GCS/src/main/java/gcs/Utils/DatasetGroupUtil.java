...
public class DatasetGroupUtil {
    ...
    public static void deleteDG(int dg_id, long mpegfile, String auth){
        ...
        System.out.println("DatasetGroupUtil.java (GCS) -> dgPath: " + dgPath);
    ...
    private static int deleteAllKeys(long mpegfile, int dg_id, String auth) {
        ...
        try (InputStream is = ProtectionUtil.class
                                  .getClassLoader()
                                  .getResourceAsStream("app.properties")) {
            if (is == null) {
                Logger.getLogger(DatasetGroupUtil.class.getName())
                      .severe("No se encontró app.properties");
                return -1;
            }
            props.load(is);
        } catch (IOException ex) {
            Logger.getLogger(DatasetGroupUtil.class.getName())
                  .log(Level.SEVERE, "Error leyendo app.properties", ex);
            return -1;
        }
        ...
        try {
            // 2) Abro conexión
            String base = props.getProperty("ps.url");
            URL url = new URL(base + "/api/v1/deleteAllKeys?dg_id=" + dg_id + "&mpegfile=" + mpegfile);
            conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("DELETE");
            conn.setRequestProperty("Authorization", "Bearer " + auth);

            // 3) Chequeo código HTTP
            int status = conn.getResponseCode();
            if (status == HttpURLConnection.HTTP_OK) {
                // 4) Leo la respuesta como entero
                try (BufferedReader br = new BufferedReader(
                    ...
                    try {
                        ...
                        Logger.getLogger(DatasetGroupUtil.class.getName())
                              .log(Level.SEVERE, "Respuesta no numérica: " + sb, nfe);
                        return -1;
                    }
                }
            } else {
                // 5) Error HTTP: no decodificamos, solo logueamos
                Logger.getLogger(DatasetGroupUtil.class.getName())
                      .warning("deleteAllKeys fallo HTTP " + status);
                try (BufferedReader br = new BufferedReader(
                    ...
                    Logger.getLogger(DatasetGroupUtil.class.getName())
                          .warning("Detalle error PS: " + sb);
                }
                return -1;
            }
        } catch (IOException ex) {
            Logger.getLogger(DatasetGroupUtil.class.getName())
                  .log(Level.SEVERE, "IOException en deleteAllKeys", ex);
            return -1;
        } finally {
            if (conn != null) {
                conn.disconnect();
